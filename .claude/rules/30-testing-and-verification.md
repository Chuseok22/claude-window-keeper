...
- `TestRunTarget_NotifiesOnSuccessfulVerification`, `TestRunTarget_SuccessNotifyDisabled_SendsNoNotification`
  (`internal/scheduler/scheduler_test.go`, Discord 성공 알림 추가분)도 각각 실제 `postPingGrace`(15초)를
  그대로 기다려서 ~15-20초씩 걸립니다.

위에 나열한 느린 테스트를 빼면 전부 500ms 이하입니다. [Issue #5](https://github.com/Chuseok22/claude-window-keeper/issues/5)에
